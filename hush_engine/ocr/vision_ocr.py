...
# Import preprocessing module (optional)
try:
    from hush_engine.preprocessing import preprocess_for_ocr
...
class VisionOCR:
    ...
    def __init__(
        ...
        Args:
            ...
            preprocess: If True, apply OpenCV preprocessing (adaptive thresholding,
                       noise removal) to improve OCR accuracy on scanned documents
        """
        ...
        self.preprocess = preprocess and PREPROCESSING_AVAILABLE
    ...
    def extract_text(self, image_path: str) -> List[TextDetection]:
        ...
        # Apply preprocessing if enabled (improves OCR on scanned/noisy documents)
        ...
        if self.preprocess:
            try:
                ...
                if processed_path != image_path:
                    temp_file = processed_path  # Track for cleanup
            except Exception as e:
                import sys
                sys.stderr.write(f"[VisionOCR] Preprocessing failed, using original: {e}\n")
                processed_path = image_path
        ...
        try:
            ...
            image_url = NSURL.fileURLWithPath_(processed_path)
